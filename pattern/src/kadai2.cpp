...
void test()
{
  ...
  ofstream ofs("../result/kadai2_result.txt");
  if(ofs.fail())
    {
      cerr << "file open error!" << endl;
      exit(1);
...
void plot(){
  ...
  if (w(1,0) != w(1,1)){
    a = (w_inverse(0,1) - w_inverse(0,0)) / (w_inverse(1,0) - w_inverse(1,1));
    fprintf(gp, "replot %lf*x\n", a);
  } else {
    fprintf(gp, "replot x = 0\n");
  }
  if (w(1,0) != w(1,1)){
  ...
  fprintf(gp, "set out \"../result/kadai2_result.png\"\n");
  fprintf(gp, "replot\n"); 
...
void inverse(){
  ...
  buf = DATA * DATA.transpose();
  buf = buf.inverse();
  w_inverse = buf * DATA * T;
  cout << "w_inverse : " << w_inverse << endl;
}
...
void test_inverse(){
  ...
  for (i=0;i<20;i++){
    g(0,0) = w_inverse.transpose().row(0) * testDATA.col(i);
    g(1,0) = w_inverse.transpose().row(1) * testDATA.col(i);
